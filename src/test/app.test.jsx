...
Objective:
The App function is the main component of the application, responsible for rendering the different screens based on the user's authentication status and role. It also initializes the Navbar component and retrieves the user's email and role from local storage.

Inputs:
- None

Flow:
1. Retrieve the existToken value from local storage or set it to false if it doesn't exist.
2. Call the useNavbar hook to retrieve the user's email and role.
3. Render the Navbar component if existToken is true.
4. Render different screens based on the user's authentication status and role using the Routes and Route components from react-router-dom.

Outputs:
- Rendered components based on the user's authentication status and role.

Additional aspects:
- The function uses the ProtectedRoute and ProtectedLogin components to prevent unauthorized access to certain screens.
- The function uses the useParams hook to retrieve the id and companyName parameters from the URL in the InventoryScreen component.
- The function uses the getLocalStorage function to retrieve values from local storage.
*/
